    ...
    <v-row class="match-height">
      ...
      >
        <v-card>
          ...
          <v-simple-table
            v-if="Object.keys(list.test_question).length > 0"
            height="auto"
          ...
          >
            <template v-slot:default>
              ...
              <tbody>
                <tr
                  v-for="(item, i) in list.test_question"
                  :key="i"
                >
                  ...
                  <td class="text-center tw-w-32">
                    <div v-if="$vuetify.breakpoint.smAndUp">
                      <v-btn
                        icon
                        :to="{ name: 'editTestQuestion', params: { id: item.id } }"
                        color="#FBBF24"
                      >
...
import { listTestQuestion, deleteTestQuestion } from '@/api/testQuestion'
...
export default {
  ...
  methods: {
    ...
    async handleSearch() {
      await this.getListTestQuestion()
    },
    ...
    async getListTestQuestion() {
      this.loading.get_data = true
      const res = await listTestQuestion({
      ...
      if (data.status || data.success) {
        ...
        this.list.test_question = data.data
      } else {
        this.loading.get_data = false
      }
    },
